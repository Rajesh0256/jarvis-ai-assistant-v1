"""
Chatbot with Server Support
This version can use either the backend server OR local API keys
"""
...
USE_SERVER = env_vars.get("USE_SERVER", "false").lower() == "true"
SERVER_URL = env_vars.get("SERVER_URL", "http://localhost:5000")
...
def ChatBotServer(Query):
    """Use backend server for AI responses"""
    try:
        with open(r"Data\ChatLog.json", "r") as f:
            messages = load(f)
        ...
        response = requests.post(
            ...
            json={
                "message": Query,
                "history": SystemChatBot + [{"role": "system", "content": RealtimeInformation()}] + messages
            },
            timeout=30
        )
        ...
        return "Cannot connect to server. Please check if server is running or switch to local mode."
    except Exception as e:
        print(f"Error: {e}")
        return "An error occurred, please try again."

def ChatBotLocal(Query):
    """Use local API keys for AI responses"""
    try:
        with open(r"Data\ChatLog.json", "r") as f:
            messages = load(f)
...
def ChatBot(Query):
    """Main chatbot function - routes to server or local based on config"""
    if USE_SERVER:
        return ChatBotServer(Query)
    else:
        return ChatBotLocal(Query)

if __name__ == "__main__":
    mode = "SERVER" if USE_SERVER else "LOCAL"
